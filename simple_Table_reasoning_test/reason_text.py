...
parser.add_argument("--input_program_file", type=str, default="fetaqa_reason_sql.json")
...
row_col=row_col_dict#历史信息传过来 ，过滤gen 只看预测是哪集行row
pattern_row = '(f_row\(\[(.*?)\]\))'
pattern_row_num = '\d+'
...
# Additional Evidence
    ...
    while len(tokenizer.encode(prompt)) >= max_prompt_tokens:
        ...
        prompt = few_shot_prompt + "\n\n" + generate_prompt
    print(f"Process#pdi 0: Building prompt for eid#{g_eid}, original_id#{g_data_item['id']}")
