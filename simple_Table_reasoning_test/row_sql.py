...
parser.add_argument(
    "--input_program_file", type=str, default="fetaqa_col_text.json"
)
...
filtered_pred = [value for value in g_data_item['table'].columns if value in pred]#保证预测出的列一定要是在表头里有
...
print(f"正在处理进程1: Building prompt for eid#{g_eid}, original_id#{g_data_item['id']}")
