...
class NeuralDB(object):
    ...
    def execute_query(self, sql_query: str):
        ...
        if len(sql_query.split(' ')) == 1 or (sql_query.startswith('`') and sql_query.endswith('`')):#特殊情况 输入是列名或带引号的值
            col_name = sql_query
    ...
    def add_sub_table(self, sub_table, join = 'normal', table_name=None, verbose=True):
        """
        ...
        # print(sub_table_df_normed)
        # #######################
        old_col = old_table.columns.tolist()
        new_col = sub_table_df_normed.columns.tolist()
        print(new_col)
        column_to_remove = 'row_id'
        if column_to_remove in old_col:
            old_col.remove(column_to_remove)
        
        drop_lst = [value for value in new_col if value in old_col]
        sub_table_df_normed.drop(columns=drop_lst, inplace=True)
        # print(sub_table_df_normed)
        # ######################
        if join == 'normal':
        ...
        if join == 'ordered':
            new_table = pd.merge(sub_table_df_normed, old_table,
                                 on='row_id', how = 'left')
            new_table.drop(columns=['index', 'row_id'], inplace = True)
            # print(new_table)
