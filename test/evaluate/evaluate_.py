...
output=dict()
with open (r'D:/project/gittest.text/TbaleReasoningWithLLM/test/evaluate/reason_text.json','r') as f:
    output = json.load(f)
    end=[]
    start=time.time()
    ...
    for key in output:
        ...
        print(f'对于第{int(key)}个问题，得到的分数是{result}')
        out_put_dict[key] = {
    ...
    #保存结果到json文件
    with open(r'D:\\project\\gittest.text\\TbaleReasoningWithLLM\\test\\evaluate\\output.json1', 'w', encoding='utf-8') as f:
