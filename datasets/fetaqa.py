...
def fetaqa_loading():
    ...
    # 指定数据文件路径，这里需要换成你本地实际路径
    filepath = os.path.join("data", "fetaQA-v1_test.json")
    data_dir = "datasets"

    # 调用生成器，遍历前几个样本
    for i, (key, example) in enumerate(fetaqa._generate_examples(filepath=filepath, data_dir=data_dir)):
