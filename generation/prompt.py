...
class PromptBuilder(object):
    ...
    def _select_x_prompt(self, df: pd.DataFrame, num_rows: int,
        ...
        for row_id, row in df.iloc[:num_rows].iterrows():
            for column_id, header in enumerate(df.columns):
                string += str(row[header])#把每一行的数据塞进去 ，用制表符来间隔
                if column_id != len(df.columns) - 1:#一行（一个样本）数据塞满 ，开始插入下一行数据
                    string += '\t'
            string += '\n'
        string += '*/\n'
        col_list = df.columns.values.tolist()
    ...
    def build_generate_prompt(
            self,
            generate_type: Tuple,
            table: pd.DataFrame,#data_item中唯一的df类
            question: str = None,
            title: str = None,
            # sub_title: str = None,
