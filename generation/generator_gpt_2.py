...
class Generator(object):
    ...
    def build_generate_prompt(self, data_item: Dict, generate_type: Tuple, **kwargs):
        """
        Build the generate prompt
        """
        return self.prompt_builder.build_generate_prompt(
            **data_item, generate_type=generate_type, **kwargs
        )
    ...
    def generate_one_pass(self, prompts: List[Tuple], verbose: bool = False):
        ...
        result_idx_to_eid = []
        for p in prompts:
            result_idx_to_eid.extend([p[0]] * self.args.sampling_n)#Extend list by appending elements from the iterable 假设有两组prompt，每个prompt采样3次结果，则存储索引的数组result_idx_to_eid就是[0, 0, 0, 1, 1, 1]这种形式
